...
from sdl2 import SDL_MOUSEMOTION, SDL_MOUSEBUTTONDOWN, SDL_BUTTON_LEFT, SDL_BUTTON_RIGHT, SDL_KEYDOWN, SDLK_SPACE, \
    SDLK_BACKSPACE, SDLK_e, SDLK_q, SDLK_w
...
class Player:
    ...
    def handle_event(self, event):
        if event.type == SDL_MOUSEMOTION:
            ...
            self.spawn_knight(self.cursor_x, self.cursor_y)
        elif event.type == SDL_KEYDOWN and event.key == SDLK_w:
            self.spawn_mage(self.cursor_x, self.cursor_y)
        elif event.type == SDL_KEYDOWN and event.key == SDLK_e:
            self.spawn_bowman(self.cursor_x, self.cursor_y)
